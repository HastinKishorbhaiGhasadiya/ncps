# Copyright 2022 Mathias Lechner and Ramin Hasani
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

import torch
import torch.nn as nn
import numpy as np
import torch.nn.functional as F

from .cfc_cell import LeCun
from .ternary import TernaryLinear, ternary_quantize


class TernaryCfCCell(nn.Module):
    """A Closed-form Continuous-time cell with ternary weight quantization.

    Mirrors :class:`ncps.torch.CfCCell` but replaces all ``nn.Linear`` layers
    with :class:`TernaryLinear`.
    """

    def __init__(
        self,
        input_size,
        hidden_size,
        mode="default",
        backbone_activation="lecun_tanh",
        backbone_units=128,
        backbone_layers=1,
        backbone_dropout=0.0,
        sparsity_mask=None,
        quantize=True,
        threshold_factor=0.7,
    ):
        super(TernaryCfCCell, self).__init__()

        self.input_size = input_size
        self.hidden_size = hidden_size
        self.quantize = quantize
        self.threshold_factor = threshold_factor

        allowed_modes = ["default", "pure", "no_gate"]
        if mode not in allowed_modes:
            raise ValueError(
                f"Unknown mode '{mode}', valid options are {str(allowed_modes)}"
            )
        self.sparsity_mask = (
            None
            if sparsity_mask is None
            else torch.nn.Parameter(
                data=torch.from_numpy(np.abs(sparsity_mask.T).astype(np.float32)),
                requires_grad=False,
            )
        )

        self.mode = mode

        if backbone_activation == "silu":
            backbone_activation = nn.SiLU
        elif backbone_activation == "relu":
            backbone_activation = nn.ReLU
        elif backbone_activation == "tanh":
            backbone_activation = nn.Tanh
        elif backbone_activation == "gelu":
            backbone_activation = nn.GELU
        elif backbone_activation == "lecun_tanh":
            backbone_activation = LeCun
        else:
            raise ValueError(f"Unknown activation {backbone_activation}")

        self.backbone = None
        self.backbone_layers = backbone_layers
        if backbone_layers > 0:
            layer_list = [
                TernaryLinear(
                    input_size + hidden_size,
                    backbone_units,
                    quantize=quantize,
                    threshold_factor=threshold_factor,
                ),
                backbone_activation(),
            ]
            for i in range(1, backbone_layers):
                layer_list.append(
                    TernaryLinear(
                        backbone_units,
                        backbone_units,
                        quantize=quantize,
                        threshold_factor=threshold_factor,
                    )
                )
                layer_list.append(backbone_activation())
                if backbone_dropout > 0.0:
                    layer_list.append(torch.nn.Dropout(backbone_dropout))
            self.backbone = nn.Sequential(*layer_list)

        self.tanh = nn.Tanh()
        self.sigmoid = nn.Sigmoid()
        cat_shape = int(
            self.hidden_size + input_size if backbone_layers == 0 else backbone_units
        )

        self.ff1 = TernaryLinear(
            cat_shape,
            hidden_size,
            quantize=quantize,
            threshold_factor=threshold_factor,
        )
        if self.mode == "pure":
            self.w_tau = torch.nn.Parameter(
                data=torch.zeros(1, self.hidden_size), requires_grad=True
            )
            self.A = torch.nn.Parameter(
                data=torch.ones(1, self.hidden_size), requires_grad=True
            )
        else:
            self.ff2 = TernaryLinear(
                cat_shape,
                hidden_size,
                quantize=quantize,
                threshold_factor=threshold_factor,
            )
            self.time_a = TernaryLinear(
                cat_shape,
                hidden_size,
                quantize=quantize,
                threshold_factor=threshold_factor,
            )
            self.time_b = TernaryLinear(
                cat_shape,
                hidden_size,
                quantize=quantize,
                threshold_factor=threshold_factor,
            )
        self.init_weights()

    def init_weights(self):
        for w in self.parameters():
            if w.dim() == 2 and w.requires_grad:
                torch.nn.init.xavier_uniform_(w)

    def forward(self, input, hx, ts):
        x = torch.cat([input, hx], 1)
        if self.backbone_layers > 0:
            x = self.backbone(x)
        if self.sparsity_mask is not None:
            w_q = ternary_quantize(self.ff1.weight, self.threshold_factor)
            w_scaled = w_q * self.ff1.scale.unsqueeze(1)
            ff1 = F.linear(x, w_scaled * self.sparsity_mask, self.ff1.bias)
        else:
            ff1 = self.ff1(x)
        if self.mode == "pure":
            new_hidden = (
                -self.A
                * torch.exp(-ts * (torch.abs(self.w_tau) + torch.abs(ff1)))
                * ff1
                + self.A
            )
        else:
            if self.sparsity_mask is not None:
                w_q2 = ternary_quantize(self.ff2.weight, self.threshold_factor)
                w_scaled2 = w_q2 * self.ff2.scale.unsqueeze(1)
                ff2 = F.linear(x, w_scaled2 * self.sparsity_mask, self.ff2.bias)
            else:
                ff2 = self.ff2(x)
            ff1 = self.tanh(ff1)
            ff2 = self.tanh(ff2)
            t_a = self.time_a(x)
            t_b = self.time_b(x)
            t_interp = self.sigmoid(t_a * ts + t_b)
            if self.mode == "no_gate":
                new_hidden = ff1 + t_interp * ff2
            else:
                new_hidden = ff1 * (1.0 - t_interp) + t_interp * ff2
        return new_hidden, new_hidden

    def get_ternary_stats(self):
        """Returns per-layer ternary weight distribution for all TernaryLinear modules."""
        stats = {}
        for name, module in self.named_modules():
            if isinstance(module, TernaryLinear):
                stats[name] = module.get_weight_distribution()
        return stats

    def get_compression_ratio(self):
        """Compute compression ratio: FP32 bits vs ternary bits.

        Ternary encoding uses 2 bits/weight + 32 bits/scale per output channel.
        """
        fp32_bits = 0
        ternary_bits = 0
        for module in self.modules():
            if isinstance(module, TernaryLinear):
                num_weights = module.weight.numel()
                out_features = module.out_features
                fp32_bits += num_weights * 32
                ternary_bits += num_weights * 2 + out_features * 32
        if ternary_bits == 0:
            return 1.0
        return fp32_bits / ternary_bits
